...
from clarifai_model_whole import ImageNet, ClarifaiModel

from l2_attack import CarliniL2
from l2_attack_black_clarifai import BlackBoxL2
...
def generate_data(data, samples, targeted=True, start=0, inception=False):
    ...
    for i in range(samples):
        if targeted:
            if inception:
                # for inception, randomly choose 2 target classes
                seq = [1]
                # seq = [580] # grand piano
            else:
...
def main(args):
    with tf.Session() as sess:
        ...
        if args['attack'] == "white":
            ...
            attack = BlackBoxL2(sess, model, batch_size=45, max_iterations=args['maxiter'], print_every=args['print_every'], 
                     early_stop_iters=args['early_stop_iters'], confidence=0, learning_rate = args['lr'], initial_const=args['init_const'], 
