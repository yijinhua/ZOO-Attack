...
class BlackBoxL2:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
        ...
        print("mystery", self.real_modifier.shape)
        ...
        print(self.loss2.shape)
        self.loss1 = self.const*loss1
        print(self.loss1.shape)
    ...
    def fake_blackbox_optimizer(self):
        img_arr = self.sess.run(self.newimg, feed_dict={self.modifier: self.real_modifier})
        scores = self.model.predict(img_arr)
        true_grads, losses, l2s, loss1, loss2, scores, nimgs = self.sess.run([self.grad_op, self.loss, self.l2dist, self.loss1, self.loss2, self.newimg], feed_dict={self.modifier: self.real_modifier, self.output: scores})
        # ADAM update
    ...
    def blackbox_optimizer(self, iteration):
        ...
        print("entering blackbox_optimizer")
        ...
        img_arr = self.sess.run(self.newimg, feed_dict={self.modifier: var})
        #print("img_arr type", type(img_arr))
        #print(img_arr.shape)
        scores = []
        for b in range(0,var.shape[0]):
            scores.append(self.model.predict(img_arr[b])[0])
        scores = np.asarray(scores)
        losses, l2s, loss1, loss2, nimgs = self.sess.run([self.loss, self.l2dist, self.loss1, self.loss2, self.newimg], feed_dict={self.modifier: var, self.output: scores})
    ...
    def attack_batch(self, img, lab, img_no):
        """
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.start_iter, self.MAX_ITERATIONS):
                ...
                if iteration%(self.print_every) == 0:
                    ...
                    inputs_arr = self.sess.run(self.newimg, feed_dict={self.modifier: self.real_modifier})
                    #print("first run", self.newimg.shape)
                    output = self.model.predict(inputs_arr)
                    #print("first out", output)
                    loss, real, other, loss1, loss2 = self.sess.run((self.loss,self.real,self.other,self.loss1,self.loss2), feed_dict={self.modifier: self.real_modifier, self.output: output})
                ...
                if iteration%(15) == 0:
                    # save image
                    show(nimg, "{}_iteration_{}.png".format(img_no, iteration))
